...
public class Perceptron
{
     ...
     *            The entire training dataset to be classified
     * @param targetVector
     *            The target values corresponding to each point in the training
     *            set
     */
    ...
    private double actualOutput(int pointIndex)
    {
        ...
        for (int i = 0; i < weightVector.length; ++i)
        {
            weightedNetInputSignal += weightVector[i]
                    * trainingData[pointIndex][i];
        }
     ...
     * 
     * @param pointIndex
     *            Iterates through the training dataset for retrieval of
     *            individual points
     * @param output
     *            actual output value
     */
    private void weightUpdate(int pointIndex, double output)
    {
        ...
        for (int i = 0; i < weightVector.length; ++i)
        {
            nextWeightVector[i] = weightVector[i]
                    + (error * trainingData[pointIndex][i]);
        }
